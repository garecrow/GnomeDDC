...
static gboolean has_prefix_casefold(const gchar *line, const gchar *prefix) {
    return g_ascii_strncasecmp(line, prefix, strlen(prefix)) == 0;
}

static gchar *combine_manufacture_info(const gchar *week, const gchar *year) {
    if (week && *week && year && *year) {
        return g_strdup_printf("Week %s, %s", week, year);
    }
    if (year && *year) {
        return g_strdup(year);
    }
    if (week && *week) {
        return g_strdup_printf("Week %s", week);
    }
    return NULL;
}

GPtrArray *ddcutil_list_monitors(GError **error) {
    const gchar *argv[] = {"ddcutil", "detect", NULL};
    gchar *output = NULL;
    ...
    gchar *manufacture_week = NULL;
    gchar *manufacture_year = NULL;
    ...
    for (gint i = 0; lines[i]; i++) {
        ...
        if (g_str_has_prefix(line, "Display")) {
            if (current) {
                ...
                if (!current->manufacture_date) {
                    current->manufacture_date = combine_manufacture_info(manufacture_week, manufacture_year);
                }
                g_ptr_array_add(monitors, current);
            }
            g_clear_pointer(&manufacture_week, g_free);
            g_clear_pointer(&manufacture_year, g_free);
        ...
        } else if (has_prefix_casefold(line, "Mfg id")) {
            g_clear_pointer(&current->manufacturer, g_free);
            current->manufacturer = extract_value(line);
        } else if (has_prefix_casefold(line, "Manufacturer")) {
            g_clear_pointer(&current->manufacturer, g_free);
            current->manufacturer = extract_value(line);
        } else if (has_prefix_casefold(line, "MCCS version")) {
            g_clear_pointer(&current->mccs_version, g_free);
            current->mccs_version = extract_value(line);
        } else if (has_prefix_casefold(line, "VCP version")) {
            g_clear_pointer(&current->mccs_version, g_free);
            current->mccs_version = extract_value(line);
        } else if (has_prefix_casefold(line, "Firmware version")) {
            g_clear_pointer(&current->firmware_version, g_free);
            current->firmware_version = extract_value(line);
        } else if (has_prefix_casefold(line, "Manufacture week")) {
            g_clear_pointer(&manufacture_week, g_free);
            manufacture_week = extract_value(line);
        } else if (has_prefix_casefold(line, "Manufacture year")) {
            g_clear_pointer(&manufacture_year, g_free);
            manufacture_year = extract_value(line);
        }
    }
...
static void clear_vcp_results(DdcutilVcpValue *results, guint n_codes) {
    if (!results) {
        return;
    }
    for (guint i = 0; i < n_codes; i++) {
...
static gboolean ddcutil_fill_multiple_values(const gchar *display_id,
                                             const guint8 *codes,
                                             guint n_codes,
    ...
    clear_vcp_results(results, n_codes);
    ...
    gboolean success = run_ddcutil((const gchar *const *)argv, &output, &local_error);
    ...
    if (!success) {
        ...
        if (local_error && local_error->domain == G_SPAWN_ERROR) {
            fallback = TRUE;
        }
        ...
        for (guint i = 0; i < n_codes; i++) {
            ...
            if (ddcutil_get_vcp_value(display_id, codes[i], &current, &maximum, &code_error)) {
                results[i].success = TRUE;
                results[i].current = current;
                results[i].maximum = maximum;
            } else {
                if (code_error && code_error->message) {
                    results[i].error_message = g_strdup(code_error->message);
                } else {
                    results[i].error_message = g_strdup("Control unavailable for this display.");
                }
                if (code_error) {
                    g_error_free(code_error);
                }
            }
        }
...
gboolean ddcutil_get_multiple_vcp_values(const gchar *display_id,
                                        ...
                                        DdcutilVcpValue *results,
                                        GError **error) {
    return ddcutil_fill_multiple_values(display_id, codes, n_codes, results, error);
}
...
void ddcutil_monitor_free(DdcutilMonitor *monitor) {
    ...
    g_free(monitor->mccs_version);
    g_free(monitor->firmware_version);
    g_free(monitor->manufacture_date);
    g_free(monitor);
}
