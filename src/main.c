...
static void picture_control_apply_value(PictureControlBinding *binding,
                                        gboolean available,
                                        gint current,
                                        gint maximum);
static void on_picture_control_value_changed(GtkRange *range, gpointer user_data);
static void on_list_view_activate(GtkListView *list_view, guint position, gpointer user_data);
...
static void on_selection_changed(GObject *selection, GParamSpec *pspec, gpointer user_data) {
    ...
    if (position == GTK_INVALID_LIST_POSITION) {
        ...
        adw_status_page_set_description(self->status_page,
                                        "Choose a display from the sidebar to adjust its settings.");
...
static void on_list_view_activate(GtkListView *list_view, guint position, gpointer user_data) {
    AppWindow *self = user_data;
    if (!self || position == GTK_INVALID_LIST_POSITION) {
        return;
    }
...
static GtkWidget *build_list_view(AppWindow *self) {
    ...
    g_signal_connect(list_view, "activate", G_CALLBACK(on_list_view_activate), self);
...
static GtkWidget *build_sidebar(AppWindow *self) {
    ...
    gtk_widget_add_css_class(app_title, "title-4");
    gtk_label_set_xalign(GTK_LABEL(app_title), 0.0f);
    gtk_box_append(GTK_BOX(title_wrapper), app_title);

    GtkWidget *category_label = gtk_label_new("Displays");
    gtk_widget_add_css_class(category_label, "title-6");
...
static GtkWidget *build_detail_panel(AppWindow *self) {
    ...
    gtk_label_set_text(self->brightness_value_label, "—");
    ...
    gtk_label_set_text(contrast_value, "—");
    ...
    gtk_label_set_text(green_value, "—");
    ...
    GtkWidget *scrolled = gtk_scrolled_window_new();
    gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(scrolled), detail_box);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolled), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
...
static void app_window_show_monitor(AppWindow *self, MonitorItem *item) {
    ...
    const gchar *codes[] = {"10", "12", "16", "18", "1A"};
    DdcutilVcpValue *values = NULL;
    GError *error = NULL;

    if (!ddcutil_get_multiple_vcp_values(display_id, codes, G_N_ELEMENTS(codes), &values, &error)) {
        app_window_set_feedback(self, error ? error->message : "Unable to query picture controls");
        if (error) {
            g_error_free(error);
        }
        app_window_update_brightness_label(self, 0);
        gtk_widget_set_sensitive(GTK_WIDGET(self->brightness_scale), FALSE);
        picture_control_apply_value(&self->contrast_control, FALSE, 0, 100);
        picture_control_apply_value(&self->red_control, FALSE, 0, 100);
        picture_control_apply_value(&self->green_control, FALSE, 0, 100);
        picture_control_apply_value(&self->blue_control, FALSE, 0, 100);
        g_free(values);
        return;
    }

    if (values && G_N_ELEMENTS(codes) > 0) {
        DdcutilVcpValue brightness = values[0];
        if (brightness.available) {
            self->updating_slider = TRUE;
            self->brightness_max = brightness.maximum > 0 ? brightness.maximum : 100;
            gtk_range_set_range(GTK_RANGE(self->brightness_scale), 0, self->brightness_max);
            gtk_range_set_value(GTK_RANGE(self->brightness_scale), brightness.current);
            self->updating_slider = FALSE;
            gtk_widget_set_sensitive(GTK_WIDGET(self->brightness_scale), TRUE);
            app_window_update_brightness_label(self, brightness.current);
        } else {
            gtk_widget_set_sensitive(GTK_WIDGET(self->brightness_scale), FALSE);
            gtk_label_set_text(self->brightness_value_label, "—");
        }

        PictureControlBinding *bindings[] = {
        ...
        for (guint i = 1; i < G_N_ELEMENTS(codes) && (i - 1) < G_N_ELEMENTS(bindings); i++) {
            picture_control_apply_value(bindings[i - 1], values[i].available, values[i].current, values[i].maximum);
        }
    }
...
static AppWindow *app_window_new(GtkApplication *app) {
    ...
    GtkWidget *window_title = adw_window_title_new("GnomeDDC", "Displays");
