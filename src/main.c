...
static GtkWidget *build_sidebar(AppWindow *self) {
    ...
    GtkWidget *category_row = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
    gtk_widget_set_hexpand(category_row, TRUE);
    ...
    gtk_box_append(GTK_BOX(category_row), category_label);

    if (self->refresh_button) {
        gtk_widget_add_css_class(GTK_WIDGET(self->refresh_button), "flat");
        gtk_widget_set_valign(GTK_WIDGET(self->refresh_button), GTK_ALIGN_CENTER);
        gtk_widget_set_halign(GTK_WIDGET(self->refresh_button), GTK_ALIGN_END);
        gtk_box_append(GTK_BOX(category_row), GTK_WIDGET(self->refresh_button));
    }

    gtk_box_append(GTK_BOX(title_wrapper), category_row);
...
static AppWindow *app_window_new(GtkApplication *app) {
    ...
    gtk_overlay_set_child(GTK_OVERLAY(overlay), split_view);
    ...
    gtk_widget_set_tooltip_text(close_button, "Close window");
    gtk_overlay_add_overlay(GTK_OVERLAY(overlay), close_button);

    g_signal_connect_swapped(close_button, "clicked", G_CALLBACK(gtk_window_close), self->window);
    self->close_button = GTK_BUTTON(close_button);

    adw_application_window_set_content(self->window, overlay);
