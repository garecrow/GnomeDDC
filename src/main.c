        ...
        for (guint g = 0; g < G_N_ELEMENTS(groups); g++) {
            if (g_strcmp0(groups[g].id, picture_control_specs[i].group_id) == 0) {
                if (!groups[g].appended) {
                    if (gtk_widget_get_first_child(detail_box) != NULL) {
                        gtk_widget_set_margin_top(groups[g].group, 12);
                    }
                    gtk_box_append(GTK_BOX(detail_box), groups[g].group);
...
static AppWindow *app_window_new(GtkApplication *app) {
    ...
    adw_window_add_breakpoint(ADW_WINDOW(self->window), breakpoint);
