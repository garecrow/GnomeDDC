...
static void app_window_apply_brightness(AppWindow *self, gint new_value);
static void on_brightness_drag_begin(GtkGestureDrag *gesture, gdouble start_x, gdouble start_y, gpointer user_data);
static void on_brightness_drag_end(GtkGestureDrag *gesture, gdouble offset_x, gdouble offset_y, gpointer user_data);
static void on_brightness_drag_cancel(GtkGesture *gesture, GdkEventSequence *sequence, gpointer user_data);
...
static void app_window_apply_brightness(AppWindow *self, gint new_value) {
    ...
    self->pending_brightness = new_value;
    ...
    g_free(message);
    self->last_sent_brightness = new_value;
}

static void on_brightness_drag_begin(GtkGestureDrag *gesture, gdouble start_x, gdouble start_y, gpointer user_data) {
    AppWindow *self = user_data;
    self->dragging_slider = TRUE;
    self->pending_brightness = (gint)gtk_range_get_value(GTK_RANGE(self->brightness_scale));
    (void)gesture;
    (void)start_x;
    (void)start_y;
}
...
static void on_brightness_drag_end(GtkGestureDrag *gesture, gdouble offset_x, gdouble offset_y, gpointer user_data) {
    ...
    gint value = (gint)gtk_range_get_value(GTK_RANGE(self->brightness_scale));
    self->pending_brightness = value;
    app_window_apply_brightness(self, value);
}
...
static void on_brightness_value_changed(GtkRange *range, gpointer user_data) {
    ...
    if (!self->dragging_slider && new_value == self->last_sent_brightness) {
        return;
    }
...
static void app_window_select_position(AppWindow *self, guint position) {
    ...
    if (position == GTK_INVALID_LIST_POSITION) {
        ...
        adw_status_page_set_description(self->status_page,
                                        "Choose a display from the sidebar to adjust its brightness.");
...
static GtkWidget *build_sidebar(AppWindow *self) {
    ...
    gtk_widget_add_css_class(list_box, "gnomeddc-sidebar-list");
...
static GtkWidget *build_detail_panel(AppWindow *self) {
    ...
    gtk_widget_add_css_class(detail_box, "gnomeddc-detail");
    ...
    gtk_widget_add_css_class(GTK_WIDGET(self->feedback_label), "gnomeddc-feedback");
    ...
    gtk_widget_add_controller(GTK_WIDGET(self->brightness_scale), GTK_EVENT_CONTROLLER(drag));
    g_signal_connect(drag, "drag-begin", G_CALLBACK(on_brightness_drag_begin), self);
    g_signal_connect(drag, "drag-end", G_CALLBACK(on_brightness_drag_end), self);
    g_signal_connect(drag, "cancel", G_CALLBACK(on_brightness_drag_cancel), self);

    GtkWidget *clamp = adw_clamp_new();
    gtk_widget_set_hexpand(clamp, TRUE);
    gtk_widget_set_vexpand(clamp, TRUE);
    gtk_widget_add_css_class(clamp, "gnomeddc-detail-clamp");
...
static void app_window_show_monitor(AppWindow *self, MonitorItem *item) {
    ...
    self->pending_brightness = current;
    self->dragging_slider = FALSE;
    self->last_sent_brightness = current;
...
static AppWindow *app_window_new(GtkApplication *app) {
    ...
    GtkWidget *window_title = adw_window_title_new("Displays", NULL);
    gtk_widget_add_css_class(window_title, "gnomeddc-page-title");
    ...
    gtk_widget_add_css_class(GTK_WIDGET(self->content_stack), "gnomeddc-content-stack");
    ...
    gtk_widget_add_css_class(content_wrapper, "gnomeddc-content");
...
static void ensure_app_styles(void) {
    ...
    if (g_once_init_enter(&initialized)) {
        const gchar *css =
            ...
            "}"
            ".gnomeddc-window .gnomeddc-sidebar-list {"
            "  background-color: transparent;"
            "  padding: 0 16px 16px 16px;"
            "}"
            ".gnomeddc-window .gnomeddc-sidebar-list row {"
            "  border-radius: 12px;"
            "  padding: 14px 18px;"
            "  margin-bottom: 8px;"
            "}"
            ".gnomeddc-window .gnomeddc-sidebar-list row:selected {"
            "  background-color: alpha(@accent_bg_color, 0.12);"
            "}"
            ".gnomeddc-window .gnomeddc-sidebar-list row:selected .dim-label {"
            "  color: @window_fg_color;"
            "}"
            ".gnomeddc-window .gnomeddc-content {"
            ...
            "  padding-bottom: 24px;"
            "}"
            ".gnomeddc-window .gnomeddc-content-stack {"
            "  padding-left: 24px;"
            "  padding-right: 24px;"
            "}"
            ".gnomeddc-window .gnomeddc-detail-clamp {"
            "  padding-top: 12px;"
            "  padding-bottom: 24px;"
            "}"
            ".gnomeddc-window .gnomeddc-detail {"
            "  background-color: transparent;"
            "}"
            ".gnomeddc-window .gnomeddc-feedback {"
            "  color: @dim_fg_color;"
            "}";
