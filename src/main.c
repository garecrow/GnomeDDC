...
static void on_rename_dialog_response(AdwAlertDialog *dialog, const gchar *response, gpointer user_data);
...
static void on_rename_button_clicked(GtkButton *button, gpointer user_data) {
    ...
    AdwDialog *dialog_base = adw_alert_dialog_new("Rename Display", NULL);
    AdwAlertDialog *dialog = ADW_ALERT_DIALOG(dialog_base);
    ...
    g_object_set_data(G_OBJECT(dialog), "rename-window", self);
    ...
    adw_dialog_present(dialog_base, GTK_WIDGET(self->window));
...
static void on_rename_entry_activated(GtkEntry *entry, gpointer user_data) {
    ...
    AppWindow *self = g_object_get_data(G_OBJECT(dialog), "rename-window");

    if (self) {
        on_rename_dialog_response(dialog, "rename", self);
    }
