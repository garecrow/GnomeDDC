...
struct _AppWindow {
    ...
    AdwActionRow *rename_row;
    GtkSizeGroup *slider_size_group;
    gboolean renaming_programmatic;
};
...
static void app_window_apply_display_name(AppWindow *self);
...
static void on_rename_apply_clicked(GtkButton *button, gpointer user_data);
...
static void app_window_update_identity(AppWindow *self, MonitorItem *item) {
    if (!self || !self->rename_entry || !self->rename_button || !self->rename_row) {
        return;
    }

    const gchar *name = item ? monitor_item_get_name(item) : "";

    self->renaming_programmatic = TRUE;
    gtk_editable_set_text(GTK_EDITABLE(self->rename_entry), name ? name : "");
    self->renaming_programmatic = FALSE;

    gboolean has_item = item != NULL;
    gtk_widget_set_sensitive(GTK_WIDGET(self->rename_row), has_item);
    gtk_widget_set_sensitive(GTK_WIDGET(self->rename_entry), has_item);
    gtk_widget_set_sensitive(GTK_WIDGET(self->rename_button), has_item);
}

static void app_window_apply_display_name(AppWindow *self) {
    if (!self || !self->rename_entry || self->current_position == GTK_INVALID_LIST_POSITION) {
        return;
    }

    const gchar *entry_text = gtk_editable_get_text(GTK_EDITABLE(self->rename_entry));
    ...
    if (!stripped || *stripped == '\0') {
        self->renaming_programmatic = TRUE;
        gtk_editable_set_text(GTK_EDITABLE(self->rename_entry), current_name ? current_name : "");
        self->renaming_programmatic = FALSE;
    ...
    self->renaming_programmatic = TRUE;
    gtk_editable_set_text(GTK_EDITABLE(self->rename_entry), stripped);
    self->renaming_programmatic = FALSE;
...
static void on_rename_apply_clicked(GtkButton *button, gpointer user_data) {
    ...
    app_window_apply_display_name(self);
    (void)button;
}

static void on_rename_entry_activated(GtkEntry *entry, gpointer user_data) {
    AppWindow *self = user_data;
    if (!self || self->renaming_programmatic) {
        return;
    }
...
static GtkWidget *build_sidebar(AppWindow *self) {
    ...
    GtkWidget *app_label = gtk_label_new("GnomeDDC");
    gtk_widget_add_css_class(app_label, "title-1");
    gtk_widget_set_halign(app_label, GTK_ALIGN_CENTER);
    gtk_widget_set_valign(app_label, GTK_ALIGN_CENTER);
    gtk_label_set_xalign(GTK_LABEL(app_label), 0.5f);
    gtk_box_append(GTK_BOX(title_wrapper), app_label);
...
static GtkWidget *build_detail_panel(AppWindow *self) {
    ...
    adw_preferences_group_set_title(ADW_PREFERENCES_GROUP(identity_group), "Identity");

    self->rename_row = ADW_ACTION_ROW(adw_action_row_new());
    g_object_set(self->rename_row, "activatable", FALSE, NULL);
    adw_preferences_row_set_title(ADW_PREFERENCES_ROW(self->rename_row), "Display name");

    GtkWidget *rename_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 12);
    gtk_widget_set_hexpand(rename_box, TRUE);

    self->rename_entry = GTK_ENTRY(gtk_entry_new());
    gtk_widget_set_hexpand(GTK_WIDGET(self->rename_entry), TRUE);
    gtk_box_append(GTK_BOX(rename_box), GTK_WIDGET(self->rename_entry));

    self->rename_button = GTK_BUTTON(gtk_button_new_with_label("Rename"));
    gtk_box_append(GTK_BOX(rename_box), GTK_WIDGET(self->rename_button));

    adw_action_row_add_suffix(self->rename_row, rename_box);
    adw_preferences_group_add(ADW_PREFERENCES_GROUP(identity_group), GTK_WIDGET(self->rename_row));

    gtk_widget_set_sensitive(GTK_WIDGET(self->rename_row), FALSE);
    gtk_widget_set_sensitive(GTK_WIDGET(self->rename_entry), FALSE);
    gtk_widget_set_sensitive(GTK_WIDGET(self->rename_button), FALSE);

    g_signal_connect(self->rename_button, "clicked", G_CALLBACK(on_rename_apply_clicked), self);
    g_signal_connect(self->rename_entry, "activate", G_CALLBACK(on_rename_entry_activated), self);

    gtk_box_append(GTK_BOX(detail_box), identity_group);
    ...
    for (guint i = 0; i < PICTURE_CONTROL_COUNT; i++) {
        ...
        gtk_size_group_add_widget(self->slider_size_group, suffix);
...
static AppWindow *app_window_new(GtkApplication *app) {
    ...
    GtkWidget *header_title = gtk_label_new("GnomeDDC");
    gtk_widget_add_css_class(header_title, "title-1");
    ...
    adw_header_bar_set_title_widget(ADW_HEADER_BAR(header_bar), header_title);
    ...
    gtk_box_append(GTK_BOX(root_box), header_bar);
    gtk_box_append(GTK_BOX(root_box), split_view);

    adw_application_window_set_content(self->window, root_box);
