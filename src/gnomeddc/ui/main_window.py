...
class MonitorPage(Gtk.Box):
    ...
    @property
    def title(self) -> str:
        return self.state.descriptor.display_label
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, app: Adw.Application, client: DdcutilClient, store: MonitorStore) -> None:
        ...
        self._content_stack.add_named(self._page_stack, "monitors")

        self._placeholder_page = Adw.StatusPage(
            title="Waiting for displays",
            description="Detecting DDC capable monitors…",
        )
        self._placeholder_page.set_icon_name("video-display-symbolic")
        self._content_stack.add_named(self._placeholder_page, "placeholder")
        ...
        self._title_widget = Adw.WindowTitle(title="GnomeDDC", subtitle="")
    ...
    def _build_sidebar(self) -> None:
        ...
        if monitors:
            ...
            self._sidebar.select_row(None)

    def _show_placeholder(self, description: str | None = None) -> None:
        if description:
            self._placeholder_page.set_description(description)
        self._content_stack.set_visible_child_name("placeholder")
        self._title_widget.set_title("GnomeDDC")
    ...
    def _show_page(self, state: MonitorState) -> None:
        ...
        if page.get_parent() is None:
            self._page_stack.add_named(page, state.descriptor.edid)
        self._page_stack.set_visible_child(page)
        self._content_stack.set_visible_child_name("monitors")
        self._title_widget.set_title(page.title)
        self._title_widget.set_subtitle(state.descriptor.short_edid)
    ...
    def _on_monitors_changed(self, _store: MonitorStore) -> None:
        monitors = self._store.monitors()
        active_ids = {state.descriptor.edid for state in monitors}
        for edid in list(self._pages.keys()):
            if edid not in active_ids:
                page = self._pages.pop(edid)
                if page.get_parent() is not None:
                    self._page_stack.remove(page)
        self._build_sidebar()
        if not monitors:
            self._show_placeholder("No DDC monitors detected. Ensure ddcutil-service is running and the display supports DDC/CI.")
        else:
            visible = self._page_stack.get_visible_child()
            if isinstance(visible, MonitorPage) and visible.state.descriptor.edid in active_ids:
                self._title_widget.set_title(visible.title)
                self._title_widget.set_subtitle(visible.state.descriptor.short_edid)
            elif monitors:
                self._show_page(monitors[0])

    def _on_row_selected(self, _listbox: Gtk.ListBox, row: Gtk.ListBoxRow) -> None:
        if not row:
            if not self._store.monitors():
